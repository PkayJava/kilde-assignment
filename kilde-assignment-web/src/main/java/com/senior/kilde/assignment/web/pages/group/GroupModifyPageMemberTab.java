package com.senior.kilde.assignment.web.pages.group;
...
public class GroupModifyPageMemberTab extends ContentPanel {
    ...
    protected void onInitData() {
        ...
        String not_in = "SELECT " + Sql.column(UserGroup_.userId) + " FROM " + Sql.table(UserGroup_.class) + " WHERE " + Sql.column(UserGroup_.groupId) + " = :group_id";
        this.user_provider = new Select2ChoiceProvider(Sql.table(User_.class), Sql.column(User_.id), Sql.column(User_.login));
        this.user_provider.applyWhere("UserGroup", Sql.column(User_.id) + " NOT IN (" + not_in + ")", Map.of("group_id", this.uuid));
