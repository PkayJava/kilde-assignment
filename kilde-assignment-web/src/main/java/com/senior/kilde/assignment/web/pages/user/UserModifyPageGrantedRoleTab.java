...
import com.senior.kilde.assignment.dao.repository.RoleRepository;
...
public class UserModifyPageGrantedRoleTab extends ContentPanel {
    ...
    protected void onInitData() {
        ...
        String not_in = "SELECT " + Sql.column(UserRole_.roleId) + " FROM " + Sql.table(UserRole_.class) + " WHERE " + Sql.column(UserRole_.userId) + " = :user_id";
        this.role_provider = new Select2ChoiceProvider(Sql.table(Role_.class), Sql.column(Role_.id), Sql.column(Role_.name));
        this.role_provider.applyWhere("UserRole", Sql.column(Role_.id) + " NOT IN (" + not_in + ")", Map.of("user_id", this.uuid));
        ...
        this.role_browse_provider.applyWhere("Group", Sql.column(UserRole_.userId) + " = :user_id", Map.of("user_id", this.uuid));
    ...
    protected void onInitHtml(MarkupContainer body) {
        ...
        this.role_column = this.row1.newUIColumn("role_column", Size.Twelve_12);
