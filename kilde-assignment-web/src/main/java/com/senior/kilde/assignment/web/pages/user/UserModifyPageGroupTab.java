...
import com.senior.kilde.assignment.web.data.Select2ChoiceProvider;
...
public class UserModifyPageGroupTab extends ContentPanel {
    ...
    protected void onInitData() {
        ...
        String not_in = "SELECT " + Sql.column(UserGroup_.groupId) + " FROM " + Sql.table(UserGroup_.class) + " WHERE " + Sql.column(UserGroup_.userId) + " = :user_id";
        this.group_provider = new Select2ChoiceProvider(Sql.table(Group_.class), Sql.column(Group_.id), Sql.column(Group_.name));
        this.group_provider.applyWhere("GroupRole", Sql.column(Group_.id) + " NOT IN (" + not_in + ")", Map.of("user_id", this.uuid));
        ...
        this.group_browse_provider.applyWhere("User", Sql.column(UserGroup_.userId) + " = :user_id", Map.of("user_id", this.uuid));
        this.group_browse_provider.setSort("group", SortOrder.ASCENDING);
    ...
    protected void onInitHtml(MarkupContainer body) {
        ...
        this.group_column = this.row1.newUIColumn("group_column", Size.Twelve_12);
