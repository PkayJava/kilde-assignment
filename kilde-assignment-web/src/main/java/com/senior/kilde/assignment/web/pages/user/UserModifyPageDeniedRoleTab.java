...
public class UserModifyPageDeniedRoleTab extends ContentPanel {
    ...
    protected void onInitData() {
        ...
        String not_in = "SELECT " + Sql.column(DenyRole_.roleId) + " FROM " + Sql.table(DenyRole_.class) + " WHERE " + Sql.column(DenyRole_.userId) + " = :user_id";
        this.role_provider = new Select2ChoiceProvider(Sql.table(Role_.class), Sql.column(Role_.id), Sql.column(Role_.name));
        this.role_provider.applyWhere("UserDeny", Sql.column(Role_.id) + " NOT IN (" + not_in + ")", Map.of("user_id", this.uuid));
        ...
        this.role_browse_provider.applyWhere("Group", Sql.column(DenyRole_.userId) + " = :user_id", Map.of("user_id", this.uuid));
